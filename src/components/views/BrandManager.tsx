...
import { Plus, Edit2, Trash2, X, Check, Globe, Folder } from 'lucide-react'
...
export default function BrandManager() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    if (editingId) {
      await supabase.from('brands').update({ name: form.name, color: form.color, drive_folder_url: form.drive_folder_url || null }).eq('id', editingId)
    } else {
      await supabase.from('brands').insert({ name: form.name, color: form.color, drive_folder_url: form.drive_folder_url || null })
    }
  ...
  const startEdit = (b: Brand) => {
    ...
    setEditingId(b.id); setShowCreate(true)
  }

  const deleteBrand = async (id: string) => {
    if (!confirm('Delete this brand? All linked briefs and tasks will be unlinked.')) return
    setDeleting(id)
    await supabase.from('brands').delete().eq('id', id)
    await load(); setDeleting(null)
  }

  const cancel = () => { setShowCreate(false); setEditingId(null); setForm({ name: '', color: '#7C3AED', drive_folder_url: '' }) }

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>Brand Management</div>
        ...
        <div style={{ background: '#fff', border: '1.5px solid var(--accent)', borderRadius: 20, padding: 24, marginBottom: 20, boxShadow: '0 4px 24px rgba(124,58,237,0.10)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            <div>
              ...
              <input style={inputStyle} placeholder="e.g. Nike India" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
            ...
            <div>
              ...
              <input style={inputStyle} placeholder="Paste Drive folder URL" value={form.drive_folder_url} onChange={e => setForm({ ...form, drive_folder_url: e.target.value })}
                onFocus={e => { (e.target as HTMLInputElement).style.borderColor = 'var(--accent)'; (e.target as HTMLInputElement).style.background = '#fff' }}
                onBlur={e => { (e.target as HTMLInputElement).style.borderColor = '#E5E5EA'; (e.target as HTMLInputElement).style.background = '#F5F5F7' }}
              />
            </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              {PRESET_COLORS.map(c => (
                <div key={c} onClick={() => setForm({ ...form, color: c })} style={{ width: 28, height: 28, borderRadius: '50%', background: c, cursor: 'pointer', border: form.color === c ? '3px solid #1D1D1F' : '3px solid transparent', transition: 'all 0.15s', boxShadow: form.color === c ? `0 0 0 2px #fff, 0 0 0 4px ${c}` : 'none' }} />
              ))}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 4 }}>
                <input type="color" value={form.color} onChange={e => setForm({ ...form, color: e.target.value })} style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', cursor: 'pointer', padding: 0, background: 'none' }} />
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Custom</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8, padding: '6px 14px', borderRadius: 100, background: `${form.color}15`, border: `1.5px solid ${form.color}` }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: form.color }} />
                <span style={{ fontSize: 12, fontWeight: 600, color: form.color }}>Preview</span>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button onClick={cancel} style={{ padding: '9px 18px', borderRadius: 100, background: 'transparent', color: 'var(--text-secondary)', border: '1px solid var(--border-default)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Cancel</button>
            <button onClick={save} disabled={saving || !form.name.trim()} style={{ padding: '9px 24px', borderRadius: 100, background: saving ? '#C4B5FD' : 'var(--accent)', color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-body)', boxShadow: '0 4px 12px rgba(124,58,237,0.3)' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          {brands.map(b => (
            <div key={b.id} style={{ background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 18, padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden', transition: 'all 0.2s' }}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => startEdit(b)} style={{ width: 30, height: 30, borderRadius: 8, background: '#F5F5F7', border: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.15s' }}
                  ...
                  <button onClick={() => deleteBrand(b.id)} disabled={deleting === b.id} style={{ width: 30, height: 30, borderRadius: 8, background: '#F5F5F7', border: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.15s' }}
          ...
          {brands.length === 0 && !showCreate && (
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 20, border: '1.5px dashed var(--border-default)' }}>
              ...
              <button onClick={() => setShowCreate(true)} style={{ padding: '10px 22px', borderRadius: 100, background: 'var(--accent)', color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Add First Brand</button>
            </div>
          )}
        </div>
      )}
