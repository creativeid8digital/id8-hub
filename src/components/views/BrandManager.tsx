...
const PRESET_COLORS = ['#7C3AED','#3B82F6','#10B981','#F59E0B','#EF4444','#EC4899','#06B6D4','#8B5CF6','#F97316','#14B8A6']
...
export default function BrandManager() {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/brands', {
        ...
        body: JSON.stringify({ id: editingId, name: form.name, color: form.color, drive_folder_url: form.drive_folder_url }),
      })
      const data = await res.json()
  ...
  const startEdit = (b: Brand) => {
    ...
    setEditingId(b.id); setShowCreate(true); setError(null)
  }
    ...
    <div>
      ...
      {showCreate && (
        <div style={{ background: '#fff', border: '1.5px solid var(--accent)', borderRadius: 20, padding: 24, marginBottom: 20, boxShadow: '0 4px 24px rgba(124,58,237,0.10)' }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 18 }}>{editingId ? 'Edit Brand' : 'New Brand'}</div>

          {error && <div style={{ background: '#FEE2E2', border: '1px solid #FECACA', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: '#EF4444', fontWeight: 500 }}>⚠️ {error}</div>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            <div>
              ...
              <input style={inp} placeholder="e.g. Nike India" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} autoFocus
            ...
            <div>
              ...
              <input style={inp} placeholder="Paste Drive folder URL" value={form.drive_folder_url} onChange={e => setForm({ ...form, drive_folder_url: e.target.value })}
                onFocus={e => { (e.target as HTMLInputElement).style.borderColor = 'var(--accent)'; (e.target as HTMLInputElement).style.background = '#fff' }}
                onBlur={e => { (e.target as HTMLInputElement).style.borderColor = '#E5E5EA'; (e.target as HTMLInputElement).style.background = '#F5F5F7' }}
              />
            </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          {brands.map(b => (
            <div key={b.id} style={{ background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 18, padding: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden', transition: 'all 0.2s' }}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  ><Edit2 size={13} color="var(--text-muted)" /></button>
                  <button onClick={() => deleteBrand(b.id)} style={{ width: 30, height: 30, borderRadius: 8, background: '#F5F5F7', border: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                    onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.background = '#FEE2E2'}
                    onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background = '#F5F5F7'}
                  ><Trash2 size={13} color="var(--text-muted)" /></button>
                </div>
              </div>
              {b.drive_folder_url && (
