...
export default function CampaignBoard({ brandId }: { brandId: string }) {
  ...
  useEffect(() => {
    ...
    // Real-time updates
    const sub = supabase.channel('campaigns').on('postgres_changes', { event: '*', schema: 'public', table: 'campaigns' }, fetch).subscribe()
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:22 }}>
        {[
          ...
          <div key={s.label} style={{ background:'var(--bg-surface)', border:'0.5px solid var(--border-subtle)', borderRadius:'var(--radius-md)', padding:'16px 18px', transition:'all 0.3s cubic-bezier(0.4,0,0.2,1)', cursor:'default' }}>
            ...
            <div key={col.key}>
              ...
              {cards.map(c => (
                <div key={c.id} style={{
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.cssText += ';border-color:var(--border-default);transform:translateY(-3px);box-shadow:0 12px 36px rgba(0,0,0,0.5),0 0 24px var(--accent-glow)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.cssText += ';border-color:rgba(124,58,237,0.12);transform:translateY(0);box-shadow:none' }}
                >
                  ...
                  {c.team && (
                    <div style={{ paddingLeft:8, marginBottom:10 }}>
                      <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 8px', borderRadius:100, fontSize:10, fontWeight:500, background:`${TEAM_COLORS[c.team]}18`, color: TEAM_COLORS[c.team], border:`0.5px solid ${TEAM_COLORS[c.team]}40` }}>
                        {c.team.replace('_', ' ')}
                      </span>
                    </div>
                  )}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingLeft:8 }}>
                    ...
                    <select
                      value={c.status}
                      onChange={e => moveCampaign(c.id, e.target.value)}
