...
import { Search, ListTodo, CheckCircle, ChevronRight } from 'lucide-react'

type Result = { id: string; title: string; type: 'task' | 'approval'; status: string; brand?: string }
...
export default function SearchView({ onOpenTask }: { onOpenTask: (id: string) => void }) {
  ...
  const search = useCallback(async (q: string) => {
    ...
    const all: Result[] = [
      ...(data.tasks || []).map((t: any) => ({ id: t.id, title: t.title, type: 'task' as const, status: t.status, brand: t.brands?.name })),
      ...(data.approvals || []).map((a: any) => ({ id: a.id, title: a.title, type: 'approval' as const, status: a.current_stage, brand: a.brands?.name })),
    ]
    setResults(all); setLoading(false)
  }, [])

  const TYPE_COLOR = {
    task:     { color: '#7C3AED', bg: '#EDE9FE', icon: <ListTodo size={14} color="#7C3AED" />,  label: 'Task'     },
    approval: { color: '#10B981', bg: '#ECFDF5', icon: <CheckCircle size={14} color="#10B981" />, label: 'Approval' },
  }

  return (
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>Search</div>
      <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>Search across tasks and approvals</div>
      ...
      <div style={{ position: 'relative', marginBottom: 24 }}>
        ...
        <input value={query} autoFocus
          ...
          placeholder="Search tasks, approvals…"
        ...
        <div>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {results.map(r => {
              ...
              return (
                <div key={r.id} onClick={() => r.type === 'task' && onOpenTask(r.id)}
                ...
                >
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: tc.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{tc.icon}</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 100, background: tc.bg, color: tc.color }}>{tc.label}</span>
