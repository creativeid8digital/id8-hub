...
import { ListTodo, Calendar, FileText, TrendingUp, CheckCircle, Code2, ChevronDown, ChevronUp, LogOut, Clock, Shield, Briefcase } from 'lucide-react'
...
const NAV = [
  ...
  { id: 'brands'      as ViewName, label: 'Brand Manager',   icon: Briefcase,   badge: null,  danger: false, adminOnly: false },
...
export default function Sidebar({ activeView, onViewChange, activeBrand, onBrandChange, user, isAdmin }: Props) {
  ...
  const loadBrands = () => { supabase.from('brands').select('*').then(({ data }) => { if (data) setBrands(data) }) }

  useEffect(() => {
    loadBrands()
    // Reload brands when brand manager might have changed them
    const sub = supabase.channel('brands-sidebar').on('postgres_changes', { event: '*', schema: 'public', table: 'brands' }, loadBrands).subscribe()
    ...
    <nav style={{ width: 240, minWidth: 240, background: '#fff', borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        {ddOpen && (
          <div style={{ marginTop: 4, background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            ...
            <div onClick={() => { onViewChange('brands'); setDdOpen(false) }}
              style={{ padding: '9px 12px', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: 'var(--accent)', fontWeight: 600, borderBottom: 'none', transition: 'background 0.1s' }}
              onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = '#F5F5F7'}
              onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}
            >
              + Manage Brands
        ...
        {visibleNav.map(item => {
          ...
          return (
            ...
            >
              ...
              {item.badge && <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 100, background: item.danger ? '#FEE2E2' : 'var(--accent-light)', color: item.danger ? '#EF4444' : 'var(--accent)' }}>{item.badge}</span>}
            </div>
          )
        })}
      </div>
      ...
      <div style={{ padding: 12, borderTop: '1px solid var(--border-subtle)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', borderRadius: 12, cursor: 'pointer', transition: 'background 0.15s' }}
          ...
          onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}
        >
          {user?.image ? <img src={user.image} style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0 }} alt="" /> : <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--accent)', flexShrink: 0 }}>{user?.name?.[0]}</div>}
