-- ═══════════════════════════════════════════════════════════════════
-- ID8 HUB — Phase 2 Migration (tested, clean)
-- Supabase → SQL Editor → New Query → paste all → Run
-- Safe on existing DB. Existing data untouched.
-- ═══════════════════════════════════════════════════════════════════
...
insert into admin_config (key, value) values
  ('admin_emails', '["kunal@id8.digital"]')
...
create policy "Users see own time logs" on time_logs for select
...
create policy "Users insert time logs" on time_logs for insert
  with check (auth.role() = 'authenticated' and
    user_id = (select id from users where email = auth.jwt()->>'email'));
create policy "Users update time logs" on time_logs for update
  using (auth.role() = 'authenticated' and
    user_id = (select id from users where email = auth.jwt()->>'email'));
...
create policy "Users see own notifications" on notifications for select
  using (auth.role() = 'authenticated' and
    user_id = (select id from users where email = auth.jwt()->>'email'));
create policy "Auth insert notifications" on notifications for insert
...
create policy "Admins update admin_config" on admin_config for update
  using ((select is_admin from users where email = auth.jwt()->>'email') = true);
...
create trigger brief_sections_updated_at
  before update on brief_sections for each row execute function update_updated_at();

-- Fill log_date + log_month on time_log insert
...
-- Auto-compute duration when timer is stopped
...
create trigger time_log_duration_trigger
  before update on time_logs
  for each row execute function compute_time_log_duration();
